...
export default function Groups() {
  ...
  const openGroup = useCallback(async g => {
    ...
    setPosts([]);
    // Only load history + join socket room if the user is a member
  ...
  const join = async id => {
    ...
    try {
      ...
      const { data: updated } = await api.get('/groups');
      setGroups(updated);
      // Re-open the group now that the user is a member
      const joined = updated.find(g => String(g.group_id) === String(id));
          ...
          <div className="chat-win">
            {!active ? (
              ...
              <>
                ...
                <div className="chat-msgs" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                  <div style={{ fontSize: 40 }}>🔒</div>
                  <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--gray-700)' }}>Members only</div>
                  <div style={{ fontSize: 13, color: 'var(--gray-400)', textAlign: 'center', maxWidth: 260 }}>
                    Join this group to see messages and participate in the discussion.
                  </div>
                  <button
                    className="btn btn-primary btn-sm"
                    style={{ marginTop: 4 }}
                    onClick={() => join(active.group_id)}
                  >
                    Join Group
                  </button>
                </div>
                <div className="chat-input">
                  <input
                    className="fi"
                    placeholder="Join the group to start chatting…"
                    disabled
                    style={{ flex: 1, opacity: 0.5, cursor: 'not-allowed' }} />
                  <button className="btn btn-primary" disabled style={{ opacity: 0.5, cursor: 'not-allowed' }}>Send</button>
                </div>
              </>
            ) : (
              /* ── Member view ── */
